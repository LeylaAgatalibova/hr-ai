    ...
    >
      <div className="flex flex-col xs:flex-row items-start gap-4">
        ...
        <div className="flex-shrink-0 self-center xs:self-start">
          <div className={`w-16 h-16 sm:w-20 sm:h-20 rounded-xl bg-gradient-to-br ${getScoreColor(result.score)} flex items-center justify-center text-white`}>
            <div className="text-center">
              <div className="text-xl sm:text-2xl font-bold">{result.score}%</div>
              <div className="text-xs opacity-90">Fit</div>
            </div>
          </div>
        </div>
        ...
        <div className="flex-1 min-w-0 w-full">
          ...
          <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-2 mb-3">
            <div className="min-w-0">
              <h3 className="font-bold text-base sm:text-lg truncate">{result.fileName}</h3>
              <p className="text-sm text-gray-600 mt-1 line-clamp-2">{result.summary}</p>
            </div>
            <span className={`px-3 py-1 rounded-full text-xs font-medium ${getStatusBadge(result.status).color} flex-shrink-0 self-start mt-2 sm:mt-0`}>
              {getStatusBadge(result.status).label}
            </span>
          </div>
                ...
                <ul className="space-y-1">
                  ...
                  {result.weaknesses.length > 3 && (
                    <li className="text-xs text-red-600 font-medium">
                      +{result.weaknesses.length - 3} more
                    </li>
                  )}
                </ul>
              </div>
