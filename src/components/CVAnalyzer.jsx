...
const CVAnalyzer = ({ importedFile }) => {
  ...
  // Handle imported file from EmailCVs (consolidated logic to prevent duplicates)
  useEffect(() => {
    // Priority 1: Use importedFile prop if provided
    if (importedFile && !processedFileIds.current.has(importedFile.id)) {
      // Mark as processed
      processedFileIds.current.add(importedFile.id);
      
      // Check for duplicates using functional update
      setUploadedFiles(prev => {
        const exists = prev.some(
          f => f.id === importedFile.id || (f.name === importedFile.name && f.size === importedFile.size)
        );
        if (!exists) {
      ...
      // Clear sessionStorage to prevent duplicate from there
      sessionStorage.removeItem('importedCV');
      return;
    }

    // Priority 2: Check sessionStorage only if no prop was provided (on mount)
    const importedCV = sessionStorage.getItem('importedCV');
    if (importedCV && !importedFile) {
      try {
        const cvData = JSON.parse(importedCV);
        const storageId = `session-${cvData.name}-${cvData.source || 'email'}`;
        
        // Check if we've already processed this
        if (processedFileIds.current.has(storageId)) {
          sessionStorage.removeItem('importedCV');
          return;
        }
        
        processedFileIds.current.add(storageId);
        
        const mockFile = new File(
          [cvData.content || ''],
          cvData.name || 'imported_cv.pdf',
          { type: 'application/pdf' }
        );
        extractTextFromFile(mockFile).then(textContent => {
          const fileForAnalyzer = {
            id: storageId,
            name: cvData.name || 'imported_cv.pdf',
            size: (cvData.content?.length || 0),
            file: mockFile,
            textContent: textContent.substring(0, 500) + '...'
          };
          
          // Check for duplicates before adding using functional update
          setUploadedFiles(prev => {
            const exists = prev.some(
              f => f.id === fileForAnalyzer.id || (f.name === fileForAnalyzer.name && f.size === fileForAnalyzer.size)
            );
            if (!exists) {
              return [...prev, fileForAnalyzer];
            }
            return prev;
          });
        ...
        console.error('Error importing CV from sessionStorage:', err);
        sessionStorage.removeItem('importedCV');
      }
    }
  }, [importedFile]); // Only depend on importedFile to avoid infinite loops
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-600 ml-14">
            CV-ləri real-time analiz edin və dəqiq uyğunluq nəticələri alın
          </p>
        </div>
